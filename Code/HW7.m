...
% priors:
prior.rho_1 = @(x) normpdf(x,0,1);
prior.rho_2 = @(x) normpdf(x,0,1);
prior.phi_1 = @(x) normpdf(x,0,1);
prior.phi_2 = @(x) normpdf(x,0,1);
prior.beta = @(x) normpdf(x,0,1);
prior.sigma_eps = @(x) lognpdf(x, -1/2, 2);
prior.sigma_1 = @(x) lognpdf(x, -1/2, 2);
prior.sigma_2 = @(x) lognpdf(x, -1/2, 2);
...
for i=1:8
    subplot(2,4,i)
    hist(parameters(:,i),30,'EdgeColor','none','FaceColor','c');
    title(str{i});
end
